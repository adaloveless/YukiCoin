...
`test/functional/test_framework/test_framework.py:YukiCoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_YukiCoin_cli.py --legacy-wallet',
    'interface_YukiCoin_cli.py --descriptors',
...
def main():
    ...
    enable_YukiCoind = config["components"].getboolean("ENABLE_YukiCoinD")

    if not enable_YukiCoind:
...
def run_tests(*, test_list, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, results_filepath=None):
    ...
    # Warn if YukiCoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "YukiCoind"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a YukiCoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    if shutil.disk_usage(tmpdir).free < min_space:
        print(f"{BOLD[1]}WARNING!{BOLD[0]} There may be insufficient free space in {tmpdir} to run the YukiCoin functional test suite. "
    ...
    # a hard link or a copy on any platform. See https://github.com/YukiCoin/YukiCoin/pull/27561.
...
class RPCCoverage():
    ...
    commands per `YukiCoin-cli help` (`rpc_interface.txt`).
