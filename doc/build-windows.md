...
Below are some notes on how to build YukiCoin Core for Windows.

The options known to work for building YukiCoin Core on Windows are:
...
example /usr/src/YukiCoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\YukiCoin`, for example:

    cmake --install build --prefix /mnt/c/workspace/YukiCoin
