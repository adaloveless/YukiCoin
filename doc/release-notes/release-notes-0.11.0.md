YukiCoin Core version 0.11.0 is now available from:

  <https://YukiCoin.org/bin/YukiCoin-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer YKI/kB fee to be
...
For example, add the following to `YukiCoin.conf`:
...
related to the blockchain in the YukiCoin system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows YukiCoin Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that YukiCoin Core will 
...
For an example script see [YukiCoin-submittx](https://github.com/laanwj/YukiCoin-submittx).
...
- #5334 `cf87536` libYukiCoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new YukiCoin-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only YukiCoinAmountField
- #5753 `0f44672` Add YukiCoin logo to about screen
...
- #5809 `46bfbe7` Add YukiCoin-cli man page
...
- #5849 `41259ca` contrib/init/YukiCoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename YukiCoin-Qt.app to YukiCoin-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for YukiCoin-tx)
