YukiCoin Core version 0.12.0 is now available from:

  <https://YukiCoin.org/bin/YukiCoin-core-0.12.0/>
...
installer (on Windows) or just copy over /Applications/YukiCoin-Qt (on Mac) or
YukiCoind/YukiCoin-qt (on Linux).
...
backwards-compatible with pre-0.10 versions of YukiCoin Core or other software:
...
pre-0.12 versions of YukiCoin Core or other software.

If you want to downgrade after you have done a reindex with 0.12.0 or later,
you will need to reindex when you first start YukiCoin Core version 0.11 or
earlier.
...
ECDSA signatures inside YukiCoin transactions now use validation using
[libsecp256k1](https://github.com/YukiCoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/YukiCoin/bips/blob/master/bip-0130.mediawiki)
...
Previous versions of YukiCoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of YukiCoin Core was to raise the default minimum
relay fee.

YukiCoin Core 0.12 will have a strict maximum size on the mempool. The
...
YukiCoin Core 0.12 also introduces new default policy limits on the length and
...
YukiCoin Core 0.12 nodes. YukiCoin Core will only allow replacement of
transactions which have any of their inputs' `nSequence` number set to less
than `0xffffffff - 1`.  Moreover, a replacement transaction may only be
accepted when it pays sufficient fee, as described in [BIP 125]
(https://github.com/YukiCoin/bips/blob/master/bip-0125.mediawiki).
...
Note that the wallet in YukiCoin Core 0.12 does not yet have support for
...
This allows running YukiCoind without having to do any manual configuration.
...
YukiCoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. YukiCoin Core will relay transactions with
insufficient fees depending on the setting of `-limitfreerelay=<r>` (default:
`r=15` kB per minute) and `-blockprioritysize=<s>`.

In YukiCoin Core 0.12, when mempool limit has been reached a higher minimum
...
entirely in YukiCoin Core 0.13, and it is at this time undecided whether the
...
YukiCoin Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
YukiCoin Core automatically creates a hidden service to listen on, without
manual configuration. YukiCoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if YukiCoin Core is listening, and
...
YukiCoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals YukiCoin
Core to use floating fees. By default, YukiCoin Core will use floating
fees.
...
(default: `0.0002` YKI/kB).

At all times, YukiCoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) YKI.
Furthermore, YukiCoin Core will never create transactions paying less than
...
`YukiCoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/YukiCoin/YukiCoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/YukiCoin/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/YukiCoin/bips/blob/master/bip-0065.mediawiki)
...
Then, to tunnel a SSL connection on 28995 to a RPC server bound on localhost on port 18995 do:

    stunnel -d 28995 -r 127.0.0.1:18995 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /YukiCoinrpc>
        ...
        # optional bypass YukiCoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< YukiCoinrpc:<password>
    </Location>
...
Restarting YukiCoind will no longer clear out the list of banned peers; instead
...
- #6214 `87406aa` [OSX] revert renaming of YukiCoin-Qt.app and use CFBundleDisplayName (partial revert of #6116) (Jonas Schnelli)
- #6218 `9d67b10` build/gitian misc updates (Cory Fields)
- #6269 `d4565b6` gitian: Use the new YukiCoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include YukiCoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix YukiCoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of YukiCoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #7209 `f3ad812` test: don't override YukiCoinD and YukiCoinCLI if they're set (Wladimir J. van der Laan)
...
- #6387 `11576a5` [YukiCoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add YukiCoinD_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libYukiCoinconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "YKI" (Ross Nicoll)
