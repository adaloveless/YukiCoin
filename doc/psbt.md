# PSBT Howto for YukiCoin Core

Since YukiCoin Core 0.17, an RPC interface exists for Partially Signed YukiCoin
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/YukiCoin/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for YukiCoin transactions that are not fully signed
...
#### Multisig with multiple YukiCoin Core instances
...
YukiCoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `YukiCoin-cli` is used.
...
Later, when *V* YKI has been received on *Amulti*, and Bob and Carol want to
