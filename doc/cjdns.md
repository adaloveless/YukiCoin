# CJDNS support in YukiCoin Core

It is possible to run YukiCoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of YukiCoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the YukiCoin network and individual nodes.
...
before it will work with your YukiCoin Core node. This is described in steps
...
## Run YukiCoin Core with CJDNS

Once you are connected to the CJDNS network, the following YukiCoin Core
...
When enabled, this option tells YukiCoin Core that it is running in an
...
IPv6 local network. This helps YukiCoin Core perform better address management:
...
CJDNS support was added to YukiCoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `YukiCoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/YukiCoin/YukiCoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in YukiCoin Core

There are several ways to see your CJDNS address in YukiCoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `YukiCoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `YukiCoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `YukiCoin-cli help getnodeaddresses` for details.
