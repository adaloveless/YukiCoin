...
static void SetupYukiCoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " YukiCoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The YukiCoin-tx tool is used for creating and modifying YukiCoin transactions.\n\n"
                "YukiCoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded YukiCoin transaction, or with \"-create [commands]\" to create a hex-encoded YukiCoin transaction.\n"
                "\n"
                "Usage: YukiCoin-tx [options] <hex-tx> [commands]\n"
                "or:    YukiCoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded YukiCoin transaction
