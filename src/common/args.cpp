...
const char * const YukiCoin_CONF_FILENAME = "YukiCoin.conf";
const char * const YukiCoin_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //YukiCoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", YukiCoin_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\YukiCoin
    //   new: C:\Users\Username\AppData\Local\YukiCoin
    // macOS: ~/Library/Application Support/YukiCoin
    // Unix-like: ~/.YukiCoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "YukiCoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "YukiCoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/YukiCoin";
#else
    // Unix-like
    return pathRet / ".YukiCoin";
#endif
#endif
}
