...
/** Call node.updateRwSetting() with YukiCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because YukiCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/YukiCoin/YukiCoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because YukiCoin 22.x will not try to read these.
...
//! Convert enabled/size values to YukiCoin -prune setting.
...
//! Get pruning enabled value to show in GUI from YukiCoin -prune setting.
...
//! Get pruning size value to show in GUI from YukiCoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    if (!settings.contains("DisplayYukiCoinUnit")) {
        settings.setValue("DisplayYukiCoinUnit", QVariant::fromValue(YukiCoinUnit::YKI));
    }
    ...
    if (unit.canConvert<YukiCoinUnit>()) {
        ...
        m_display_YukiCoin_unit = YukiCoinUnit::YKI;
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if YukiCoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_YukiCoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<YukiCoinUnit>() == m_display_YukiCoin_unit) return;
    m_display_YukiCoin_unit = new_unit.value<YukiCoinUnit>();
    QSettings settings;
    settings.setValue("DisplayYukiCoinUnit", QVariant::fromValue(m_display_YukiCoin_unit));
    Q_EMIT displayUnitChanged(m_display_YukiCoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/YukiCoin/YukiCoin/pull/8273
    ...
    // (https://github.com/YukiCoin-core/gui/issues/567).
