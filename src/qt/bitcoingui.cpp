...
const std::string YukiCoinGUI::DEFAULT_UIPLATFORM =
...
YukiCoinGUI::YukiCoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &YukiCoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &YukiCoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &YukiCoinGUI::showModalOverlay);
...
void YukiCoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a YukiCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and YukiCoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &YukiCoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &YukiCoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &YukiCoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your YukiCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified YukiCoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed YukiCoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed YukiCoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a YukiCoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible YukiCoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &YukiCoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &YukiCoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &YukiCoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &YukiCoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &YukiCoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &YukiCoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &YukiCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &YukiCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &YukiCoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &YukiCoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &YukiCoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &YukiCoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &YukiCoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &YukiCoinGUI::showDebugWindow);
}

void YukiCoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void YukiCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &YukiCoinGUI::setCurrentWalletBySelectorIndex);
...
void YukiCoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &YukiCoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &YukiCoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &YukiCoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &YukiCoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void YukiCoinGUI::enableHistoryAction(bool privacy)
{
...
void YukiCoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &YukiCoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &YukiCoinGUI::removeWallet);
...
WalletController* YukiCoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void YukiCoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &YukiCoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &YukiCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &YukiCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &YukiCoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &YukiCoinGUI::incomingTransaction);
    connect(this, &YukiCoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void YukiCoinGUI::removeWallet(WalletModel* walletModel)
{
...
void YukiCoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void YukiCoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void YukiCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void YukiCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void YukiCoinGUI::createTrayIcon()
{
...
void YukiCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &YukiCoinGUI::toggleHidden);
...
void YukiCoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void YukiCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void YukiCoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void YukiCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void YukiCoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void YukiCoinGUI::openClicked()
{
...
void YukiCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void YukiCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void YukiCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void YukiCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void YukiCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void YukiCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void YukiCoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void YukiCoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to YukiCoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void YukiCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void YukiCoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void YukiCoinGUI::updateHeadersSyncProgressLabel()
{
...
void YukiCoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void YukiCoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &YukiCoinGUI::quitRequested);
...
void YukiCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void YukiCoinGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &YukiCoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void YukiCoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void YukiCoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &YukiCoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &YukiCoinGUI::show);
                e->ignore();
            }
        }
    }
...
void YukiCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void YukiCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void YukiCoinGUI::incomingTransaction(const QString& date, YukiCoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(YukiCoinUnits::formatWithUnit(unit, amount, true));
...
void YukiCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void YukiCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool YukiCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool YukiCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void YukiCoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void YukiCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void YukiCoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void YukiCoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void YukiCoinGUI::updateWindowTitle()
{
...
void YukiCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void YukiCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void YukiCoinGUI::detectShutdown()
{
...
void YukiCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void YukiCoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(YukiCoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = YukiCoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void YukiCoinGUI::subscribeToCoreSignals()
{
...
void YukiCoinGUI::unsubscribeFromCoreSignals()
{
...
bool YukiCoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<YukiCoinUnit> units = YukiCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const YukiCoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, YukiCoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const YukiCoinUnit u : YukiCoinUnits::availableUnits()) {
        menu->addAction(YukiCoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(YukiCoinUnit newUnits)
{
    setText(YukiCoinUnits::longName(newUnits));
}
