...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<YukiCoinUnit>("YukiCoinUnit");
#else
    qRegisterMetaType<YukiCoinUnit>("YukiCoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. YukiCoin_de.qm (shortcut "de" needs to be defined in YukiCoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. YukiCoin_de_DE.qm (shortcut "de_DE" needs to be defined in YukiCoin.qrc)
...
static const char* qt_argv = "YukiCoin-qt";

YukiCoinApplication::YukiCoinApplication()
...
void YukiCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the YukiCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", YukiCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void YukiCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool YukiCoinApplication::createOptionsModel(bool resetSettings)
{
...
void YukiCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new YukiCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &YukiCoinGUI::quitRequested, this, &YukiCoinApplication::requestShutdown);
...
void YukiCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void YukiCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool YukiCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void YukiCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &YukiCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &YukiCoinApplication::handleRunawayException);
    connect(this, &YukiCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &YukiCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void YukiCoinApplication::parameterSetup()
{
...
void YukiCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void YukiCoinApplication::requestInitialize()
{
...
void YukiCoinApplication::requestShutdown()
{
...
void YukiCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // YukiCoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &YukiCoinGUI::handlePaymentRequest);
            connect(window, &YukiCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void YukiCoinApplication::handleRunawayException(const QString &message)
{
...
void YukiCoinApplication::handleNonFatalException(const QString& message)
{
...
WId YukiCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool YukiCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", YukiCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(YukiCoin);
    Q_INIT_RESOURCE(YukiCoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(YukiCoin_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see YukiCoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see YukiCoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse YukiCoin.conf, determine network, switch to network specific
    ...
    // YukiCoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
