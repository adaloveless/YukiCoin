# Bootstrappable YukiCoin Core Builds

This directory contains the files necessary to perform bootstrappable YukiCoin
Core builds.
...
To build YukiCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      YukiCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/YukiCoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/YukiCoin-core/guix.sigs).
