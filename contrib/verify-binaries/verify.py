...
"""Script for verifying YukiCoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from YukiCoincore.org and YukiCoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/YukiCoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://YukiCoincore.org"
HOST2 = "https://YukiCoin.org"
VERSIONPREFIX = "YukiCoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the YukiCoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"YukiCoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by YukiCoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the YukiCoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes YukiCoin.org lags behind YukiCoincore.org.)')
    )
