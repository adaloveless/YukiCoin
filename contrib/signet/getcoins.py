#!/usr/bin/env python3
# Copyright (c) 2020-2022 The YukiCoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing YukiCoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='YukiCoin-cli', help='YukiCoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='YukiCoin address to which the faucet should send')
...
parser.add_argument('YukiCoin_cli_args', nargs='*', help='Arguments to pass on to YukiCoin-cli (default: -signet)')

args = parser.parse_args()

if args.YukiCoin_cli_args == []:
    args.YukiCoin_cli_args = ['-signet']


def YukiCoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.YukiCoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = YukiCoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = YukiCoin_cli(['getnewaddress', 'faucet', 'bech32'])
