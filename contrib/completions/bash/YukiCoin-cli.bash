...
_YukiCoin_cli() {
    ...
    # save and use original argument to invoke YukiCoin-cli for -help, help and RPC
    # as YukiCoin-cli might not be in $PATH
    YukiCoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($YukiCoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_YukiCoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _YukiCoin_cli YukiCoin-cli
