...
Tracing (USDT) support for YukiCoin Core on Linux using. For more information on
USDT support in YukiCoin Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `YukiCoind` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `YukiCoind` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[YukiCoin Core USDT documentation] on how to list available tracepoints in your
`YukiCoind` binary.

[YukiCoin Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof YukiCoind)
```
...
YukiCoin P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof YukiCoind)
```
...
In a different terminal, starting YukiCoin Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/src/YukiCoind -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof YukiCoind)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof YukiCoind)
```
