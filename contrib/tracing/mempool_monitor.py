...
""" Example logging YukiCoin Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into YukiCoind with pid {pid}")
    YukiCoind_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    YukiCoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    YukiCoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    YukiCoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    YukiCoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[YukiCoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of YukiCoind>")
        exit(1)
